...
class Titlebar(QWidget):
	def __init__(self, parent):
		...
		iconWidth = self.height() - (margin := 7) * 2
		btns.setContentsMargins(margin,0,0,0)
	...
	def paintMin(self, e, btn):
		...
		if btn.underMouse():
			p.fillRect(rect, QColor(COLOR["hovered"]))
	...
	def paintMax(self, e, btn):
		p = QPainter(btn)
		rect = btn.rect()
		background = COLOR["hovered" if btn.underMouse() else "background-titlebar"]
